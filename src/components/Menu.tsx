...
export function MenuItem({ children, onClick, disabled = false, icon, isActive = false }: MenuItemProps) {
  return (
    <button
      className={`relative block w-full h-16 text-center group cursor-pointer
  ...
  return (
    <div className="relative w-[64px]" data-expanded={isExpanded}>
      ...
      <div className="relative">
        ...
        <div 
          className="relative w-16 h-16 bg-white/10 dark:bg-black/40 backdrop-blur-md cursor-pointer rounded-full group will-change-transform z-50 flex items-center justify-center shadow-lg hover:scale-105 transition-all duration-200 border border-white/10 hover:border-white/20"
        ...
        {childrenArray.slice(1).map((child, index) => (
          <div 
            key={index} 
            className="absolute top-0 left-0 w-16 h-16 bg-white/10 dark:bg-black/40 backdrop-blur-md will-change-transform rounded-full flex items-center justify-center shadow-md border border-white/5 hover:border-white/10 hover:scale-105 transition-all duration-200"
            style={{
              transform: `translateY(${isExpanded ? (index + 1) * 68 : 0}px)`, /* Adjusted offset for fully rounded circles */
